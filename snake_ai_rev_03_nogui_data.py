...
gameList = []
numGames = int(input('Enter The Number Of Games To Play: ').strip())
game = 1
while game <= numGames:
    timeStart = time.time()
    board = []
    ...
    while ongoing:
        ...
        if length == boardSize ** 2:
            ongoing = False
            print('Game {0} Won.'.format(game))
            game += 1
            timeEnd = time.time()
            timeTotal = timeEnd-timeStart
            gameList.append((moveCount, timeTotal))
allMoves = 0
allRuntime = 0
moveList = []
for i in gameList:
    ...
    allRuntime += i[1]
    moveList.append(i[0])
averageMoves = allMoves/numGames
averageRuntime = allRuntime/numGames
stdev = statistics.stdev(moveList, averageMoves)
print('-' * 50)
print('{0} games were simulated.'.format(numGames))
print('Average Number Of Moves Was {0:.2f}'.format(averageMoves))
print('Average Runtime Was {0:.2f}'.format(averageRuntime))
print('Standard Deviation Was {0:.2f}.'.format(stdev))
